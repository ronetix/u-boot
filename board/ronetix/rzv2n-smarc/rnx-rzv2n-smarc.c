...
#define PFC_P(port)		(PFC_BASE + 0x020 + port)			/*  8-bit register */
#define PFC_PM(port)	(PFC_BASE + 0x140 + (port * 2))		/* 16-bit register */
#define PFC_PMC(port)	(PFC_BASE + 0x220 + port)			/*  8-bit register */
#define PFC_PFC(port)	(PFC_BASE + 0x480 + (port * 4))		/* 32-bit register */

#define GPIO_CONF_OUTPUT(port, pin)							\
{															\
	*(volatile u32 *)PWPR |= (PWPR_REGWE_A | PWPR_REGWE_B);	\
	(*(volatile u8  *)(PFC_PMC(port)) &= ~(1 << pin));		\
	(*(volatile u16 *)(PFC_PM(port))  |=  (3 << (pin * 2)));\
	*(volatile u32 *)PWPR &= ~(PWPR_REGWE_A | PWPR_REGWE_B);\
}

#define GPIO_SET(port, pin)		(*(volatile u8 *)(PFC_P(port)) |= (1 << pin))
#define GPIO_CLEAR(port, pin)	(*(volatile u8 *)(PFC_P(port)) &= ~(1 << pin))
...
/*!
 * Reset both Ethernet PHYs
 */
static void reset_eth_phy(void)
{
	/* define P84 and P85 as outputs */
	GPIO_CONF_OUTPUT(8, 4);
	GPIO_CONF_OUTPUT(8, 5);

	GPIO_CLEAR(8, 4);		/* ETH0_PHY: RESETn = 0 */
	GPIO_CLEAR(8, 5);		/* ETH1_PHY: RESETn = 0 */

	/* Delay 10mS */
	udelay(10000);

	GPIO_SET(8, 4);			/* ETH0_PHY: RESETn = 1 */
	GPIO_SET(8, 5);			/* ETH1_PHY: RESETn = 1 */
